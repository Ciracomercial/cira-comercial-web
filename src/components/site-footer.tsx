  ...
  return (
    <footer id="contacto">
      ...
      <div className="container footer-grid">
        <div className="footer-brand"><Image className="footer-logo" src="/assets/branding/cira-logo.png" alt="Cira Comercial" width={64} height={64} /><p>Proveedor confiable de productos de limpieza, higiene y desechables en Nuevo Casas Grandes.</p><p className="footer-trust">Atención personalizada y facturación disponible.</p></div>
        <div><h3>Horario de atención</h3><p>{business.hours.weekdays.display}<br />{business.hours.saturday.display}</p></div>
        <div><h3>Contacto</h3>{mapUrl ? <a href={mapUrl} target="_blank" rel="noopener noreferrer">{business.streetAddress}<br />C.P. {business.postalCode}, {business.city}, {business.state}</a> : <p>{business.streetAddress}<br />C.P. {business.postalCode}, {business.city}, {business.state}</p>}<a href={`tel:${business.phone}`}>{business.phoneDisplay}</a><a href={`mailto:${business.email}`}>{business.email}</a></div>
        <div className="footer-contact"><a className="whatsapp-link" href={whatsappUrl} target="_blank" rel="noopener noreferrer">WhatsApp<br /><strong>{business.phoneDisplay}</strong></a></div>
